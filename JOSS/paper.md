...
DecoupleFlow is inspired by GPipe in its use of the pipeline concept, partitioning a model into sequential stages and scheduling execution across devices. Through this partition-and-distribute design, DecoupleFlow can also reduce per-device memory and computation burden, similar to the practical effect of GPipe. Beyond this overlap, DecoupleFlow targets a different objective: it refactors user-defined models into decoupled architectures with block-level components (e.g., projector heads and local objectives), providing a unified implementation approach for decoupled architectures including SCPL or DeInfoReg while retaining flexibility to incorporate future decoupled learning architectures.
